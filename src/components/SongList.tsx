...
import { toggleIsSongListOpen, decSongIndex } from '../actions/currentSessionActions'

// This is the current Playlist of songs which will be showed when user Toggles it.
function SongList() {
  const dispatch = useDispatch()
  const { playingSong, songIndex } = useSelector((state) => state.currentSession)
  ...
  const closeSongList = (event) => {
    if (event.target === songListContainerRef.current) dispatch(toggleIsSongListOpen())
  }
